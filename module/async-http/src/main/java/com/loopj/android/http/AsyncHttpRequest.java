...
public class AsyncHttpRequest implements Runnable {
    ...
    public void run() {
        ...
        if (responseHandler == null) {
            AsyncHttpClient.log.w("AsyncHttpRequest", "responseHandler has been null , finish run()");
            return;
        }
    ...
    private synchronized void sendCancelNotification() {
        if (responseHandler == null) {
            AsyncHttpClient.log.w("AsyncHttpRequest", "responseHandler has been null , no more sendCancelNotification");
            return;
        }
    ...
    public AsyncHttpRequest setRequestTag(Object TAG) {
        // XXX 在这里保存一份tag对象会导致responseHandler里的setTag()失效
        this.TAG = new WeakReference<>(TAG);
        if (responseHandler == null) {
            AsyncHttpClient.log.w("AsyncHttpRequest", "responseHandler has been null , 'responseHandler.setTag(TAG);' is invalid , use AsyncHttpRequest's backup");
        } else {
    ...
    public Object getTag() {
        if (responseHandler == null) {
            AsyncHttpClient.log.w("AsyncHttpRequest", "responseHandler has been null , 'responseHandler.getTag();' is invalid , use AsyncHttpRequest's backup");
            return this.TAG.get();
        } else {
